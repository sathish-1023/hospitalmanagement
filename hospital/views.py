...
def discharge_patient_view(request,pk):
    ...
    if request.method == 'POST':
        ...
        #return render(request, 'hospital/redirect.html', context=paytm_params)
...
def initiate_payment(request,pk):
   # if request.method == "GET":
    #    return render(request, 'hospital/pay.html')
    ...
    #except:
        #return render(request, 'hospital/pay.html', context={'error': 'Wrong Accound Details or amount'})
    dd=models.PatientDischargeDetails.objects.all().filter(patientId=pk).order_by('-id')[:1]
    amount=dd[0].total
...
def callback(request):
    if request.method == 'POST':
        ...
        return render(request, 'hospital/callback.html', context=received_data)
